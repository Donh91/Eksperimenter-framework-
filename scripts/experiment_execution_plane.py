...
def recompute_status(request: dict[str, Any]) -> str:
    ...
    if any(value is None for value in recomputed): return "REPLICATED_WAITING_FOR_DATA"
    if recomputed and all(value is True for value in recomputed): return "REPLICATED_FIRED"
...
def main() -> None:
    ...
    for path in sorted(args.receipt_root.glob("*.json")):
        ...
        rel = str(path)
        receipts.append({"receipt_id": value["receipt_id"], "candidate_id": value["candidate_id"], "path": rel, "sha256": sha256(value), "raw_url": f"https://raw.githubusercontent.com/{args.repository}/{args.branch}/{rel}", "replication_status": value["replication_status"]})
